...
- **Copilot 语法**：使用**短横线** `/opsx-propose`（不能用冒号 `/opsx:propose`，那是 Claude Code 的语法）
- **初始化命令**：`openspec init` 交互选择 `github-copilot`，命令安装到 `.github/prompts/opsx-*.prompt.md`
- **依赖**：Node.js 20.19+
...
- **逐步执行**：无内建逐步命令，`/speckit.implement` 一口气完成所有任务。如需逐任务执行，有两种方式：
  - 手动方式：生成 `tasks.md` 后，逐个对话提示 AI 实现单个任务（如"请实现任务 1.1"）
  - 宪法约束：在 `.specify/memory/constitution.md` 添加 Article X 要求每完成一个 task 暂停等待确认
